...
def getCountsAndAverages(IDandRatingsTuple):
    ...
    """
    secondkey = sum(x for x in IDandRatingsTuple[1])/float(len(IDandRatingsTuple[1]))
    return (IDandRatingsTuple[0],(len(IDandRatingsTuple[1]),secondkey))
...
movieIDsWithAvgRatingsRDD = movieIDsWithRatingsRDD.map(lambda x: (x[0],(len(x[1]), sum(x[1])/float(len(x[1])))))
...
# TODO: Replace <FILL IN> with appropriate code
trainingWithMyRatingsRDD = trainingRDD+myRatingsRDD
...
myUnratedMoviesRDD = (moviesRDD.filter(lambda x: x[0] not in [vals[1] for vals in myRatedMovies]).map(lambda y: (0, y[0])))
print myUnratedMoviesRDD.take(2)
# Use the input RDD, myUnratedMoviesRDD, with myRatingsModel.predictAll() to predict your ratings for the movies
predictedRatingsRDD = myRatingsModel.predictAll(myUnratedMoviesRDD)
print predictedRatingsRDD.take(2)
...
ratingsWithNamesRDD = (predictedWithCountsRDD
                       ...
                       .map(lambda y: (y[0],y[1][1]))
                       .join(moviesRDD)
                       .map(lambda z: (z[0], z[1][1], z[1][0]))
                       )
