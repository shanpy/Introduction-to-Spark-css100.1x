...
dayGroupedHosts = dayToHostPairTuple.groupByKey().map(lambda x : (x[0], set(list(x[1]))))
...
groupedByDay = dayAndHostTuple.groupByKey().map(lambda x : (x[0], len(list(x[1]))/len(set(list(x[1])))))
...
badEndpointsSum = badEndpointsCountPairTuple.groupByKey().map(lambda x: (x[0], sum(x[1])))
...
errHostsSum = errHostsCountPairTuple.groupByKey().map(lambda x: (x[0], sum(x[1])))
